...
- 作成日: 2026-06-10
- 本番利用: **2026-06-11（明日）**。当日までにGitHub Pages公開＋2人の実機確認まで完了させる。
- ステータス: 設計確定（実装計画 未作成）
...
## 9. 段階リリースと当日運用（2人・2端末）

### 段階リリース（明日までに全18テーブル公開が目標）
- **Phase 1（スモークテスト）**: `TABLES = ["A"]` で5画面の通しを素早く確認。
  - 撮影 → IndexedDB保存 → 一覧表示/メモ/削除 → Excel出力 → ZIP出力 が一気通貫で動くこと。
- **Phase 2（A〜R / 本番）**: `TABLES` を A〜R の18個に拡張。GitHub Pages へ公開し、
  2人の実機（スマホ）で QR読取〜出力まで確認する。← **当日の到達目標はここ**

### 当日運用モデル
- **2人がそれぞれの端末で撮影**する。データは端末ごとの IndexedDB に独立して貯まる
  （サーバー0円のため端末間自動共有はしない）。
- **テーブルは2人で分担し、重複しない**（例: 人1=A〜I、人2=J〜R）。
  - 同じテーブルを両者が撮らない前提なので、画像名・テーブルフォルダは衝突しない。
- **成果物の結合は手動（最速方式）**:
  1. 各自が自分の端末で Excel と 画像ZIP を出力する。
  2. PC上で、2つのZIPを展開して重ねる（テーブルフォルダが別々なので単純コピーで統合）。
  3. 2つのExcelの行を連結して1つにまとめる。
- アプリ側に端末間マージ機能は実装しない（YAGNI）。分担運用＋手動結合で十分なため。
